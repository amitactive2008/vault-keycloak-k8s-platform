...
KEYS_FILE="${SCRIPT_DIR}/../../vault/cluster-keys.json"
...
echo ""
echo "Next steps:"
echo "  kubectl apply -f vault-rbac.yaml          # grant Vault SA token-create perms"
echo "  kubectl apply -f serviceaccount.yaml      # create webapp-backend SA"
echo "  kubectl apply -f postgres.yaml            # deploy PostgreSQL"
echo "  kubectl apply -f backend.yaml             # deploy backend (Vault Agent injected)"
echo "  kubectl apply -f frontend.yaml            # deploy frontend"
echo "  kubectl apply -f ingress.yaml             # expose via ingress"
echo "  echo '127.0.0.1 team-a-webapp.local' | sudo tee -a /etc/hosts"
