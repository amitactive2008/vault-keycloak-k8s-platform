...
# TLS cert files (generated in this script)
TLS_DIR="/private/tmp"
CA_CERT="${TLS_DIR}/keycloak-local-ca.crt"
CA_KEY="${TLS_DIR}/keycloak-local-ca.key"
...
APISERVER_CA_FILE="/etc/kubernetes/pki/keycloak-local-ca.crt"
...
section "Step 1 — Generating self-signed TLS cert for keycloak.local"

if [ -f "$CA_CERT" ] && [ -f "$TLS_CERT" ]; then
  ...
  openssl req -x509 -new -nodes -key "${CA_KEY}" \
    ...
    -subj "/C=US/O=kind-vault/CN=keycloak-local-ca"

  info "Generating server cert for keycloak.local..."
...
# Save CA cert alongside this script for kubeconfig and user setup
